...
aggregated as (
    select
        ...
        round(min(fact.comp_yearly_usd), 0)                                  as min_salary_usd,
        ...
        round(max(fact.comp_yearly_usd), 0)                                  as max_salary_usd,
        round(avg(fact.comp_yearly_eur), 0)                                  as avg_salary_eur,
        round(min(fact.comp_yearly_eur), 0)                                  as min_salary_eur,
        ...
        round(max(fact.comp_yearly_eur), 0)                                  as max_salary_eur
    from fact
    group by 1, 2, 3, 4
)
